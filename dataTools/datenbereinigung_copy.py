...
countries_alternative_names={\
    ...
    "Timor-Leste":["Timor","Timor Leste","East Timor","East Timor *","East Timor","Timor-Leste"," East Timor"],\
...
p=pd.read_csv("data/important/countrylist.csv",index_col=False)
...
def cleancolumncountry(namecolumn,dataframe,columnstoconverttonumbers,bypopulation=False,byarea=False,killdoubles=False,killdoubles_column=0,extra_columns=[]):
    ...
    dataframe.columns=range(len(df.columns))


    # dataframe[namecolumn]=dataframe[namecolumn].apply(lambda x: str(x))
    dataframe[namecolumn]=dataframe[namecolumn].str.strip()
    dataframe[namecolumn].apply(lambda x: x.rstrip("*"))
    dataframe[namecolumn].apply(lambda x: x.rstrip(" "))
    ...
    for country in countrynamelist2:
        if not country in useless_countries:
            print(country)
    print("\n This is the number of countries which were not found:")
    ...
    for i in columnstoconverttonumbers:
        ...
        # dataframe[i]=dataframe[i].apply(lambda x: "-9999" if "None" in x  else x)
        
        dataframe[i]=dataframe[i].apply(lambda x: x.replace("m",""))
        ...
        dataframe[i]=dataframe[i].fillna(-1.0)
        pass
    ...
    print(savelist)
    saveurl="data/" + interestingurl + additional
    print(saveurl)
    dataframe=dataframe[savelist]
    dataframe.columns=list(range(len(savelist)))
    saveurl=saveurl+".csv"
    dataframe.to_csv(saveurl,index=False)
    print(dataframe)
...
# print(df)
# df.to_csv("rawdata/GDP per capita (PPP).csv",index=False)
...
#df=df[:-15]
#print(df.iloc[177][0])
df=pd.read_csv("e.csv",index_col=False)
cleancolumncountry(0,df,[1],bypopulation=True)
